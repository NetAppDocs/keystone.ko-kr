...
파일 서버 API를 사용하여 파일 서버를 보고 관리할 수 있습니다. 파일 서버에 대한 자세한 내용은 을 참조하십시오 link:sewebiug_create_a_file_server.html["파일 서버를 생성합니다"].
...
이름 : 파일 공유의 이름입니다. '공유_경로': 파일 공유의 경로입니다. 'fileserver_id': 파일 서버의 ID. size_gb: 파일 공유 크기(gibs)입니다. 'service_level': Standard, Premium, Premium-Tiering, Extreme 또는 Extreme-Tiering의 서비스 레벨 이름을 사용할 수 있습니다. '프로토콜': 파일 공유(NFS, CIFS 또는 멀티 프로토콜)를 액세스하는 데 사용되는 프로토콜입니다. 보안 스타일(Unix 또는 NTFS). Export_policy: 파일 공유의 내보내기 정책입니다.
...
id: 파일 공유의 ID입니다. 이름 : 파일 공유의 이름입니다. size_gb: 파일 공유 크기(gibs)입니다. 'service_level': Standard, Premium, Premium-Tiering, Extreme 또는 Extreme-Tiering의 서비스 레벨 이름을 사용할 수 있습니다. '프로토콜': 파일 공유(NFS, CIFS 또는 멀티 프로토콜)를 액세스하는 데 사용되는 프로토콜입니다. Export_policy: 파일 공유의 내보내기 정책입니다.
